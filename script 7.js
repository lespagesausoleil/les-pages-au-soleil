// MENU GLOBAL UNIFIE — Les Pages au Soleil
(() => {
  ...
  const isArticles =
    ...
    path.includes('rubrique-') ||
    path.includes('coin-lecture');
  const isBoutique =
  ...
  mainNav.innerHTML = `
    <a href="index.html"${isHome ? ' class="is-active"' : ''}>Accueil</a>
    <a href="articles.html"${isArticles && !isHome ? ' class="is-active"' : ''}>Articles</a>
    <a href="boutique.html"${isBoutique ? ' class="shop-nav-link is-active"' : ' class="shop-nav-link"'}>Boutique</a>
    <a href="index.html#apropos">À propos</a>
    <a href="contact.html"${isContact ? ' class="is-active"' : ''}>Contact</a>
    <button class="search-open" type="button" aria-label="Rechercher">⌕</button>
  `;

  // Certaines anciennes fiches produits n'avaient pas encore le panneau
  // de recherche moderne. On l'ajoute automatiquement si nécessaire.
